了解！第37章は「外部依存を包む（Adapter/WrapperでSeamを作る）」なので、最新の.NET/C#事情（HttpClient/IHttpClientFactory、テストのFake/Mock、SQLiteやファイルI/O周り等）をWebで確認してから、教材ページとしてそのまま使える形で作るね。
